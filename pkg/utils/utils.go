...
func getPreferredNodeAddress(node *api_v1.Node, preferredAddressTypes []api_v1.NodeAddressType) (string, error) {
	for _, addressType := range preferredAddressTypes {
		for _, address := range node.Status.Addresses {
			// TEMPORARY USE ONLY IPv6 addresses from nodes
			if address.Type == addressType && strings.Contains(address.Address, ":") {
...
func SubnetHasIPv4Range(gcecloud *gce.Cloud, subnetName string) (bool, error) {
	key := meta.RegionalKey(subnetName, gcecloud.Region())
	subnet, err := gcecloud.Compute().AlphaSubnetworks().Get(context.Background(), key)
	if err != nil {
		return false, fmt.Errorf("failed getting subnet: %w", err)
